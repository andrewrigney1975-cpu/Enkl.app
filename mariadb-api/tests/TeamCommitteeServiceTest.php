...
final class TeamCommitteeServiceTest extends TestCase
{
    ...
    public function testApplyOrgTeamCreatesTeamCommitteeAndProjectMembers(): void
    {
        ...
        // TestDataHelper's own note), unlike php-api's original which relied on Postgres's
        // `DEFAULT gen_random_uuid()`; (2) PDO_MYSQL's NATIVE prepared statements (unlike PDO_PGSQL)
        ...
        // "NormalizedUsername", which worked fine against Postgres but not here, so this needs its
        // own separate placeholder even though both bind the identical value.
        $username = TestDataHelper::unique('teammate');
        self::$db->prepare(<<<SQL
            ...
            VALUES (:id, :orgId, :username, :normalizedUsername, :hash, :displayName, false, false, true, now(), :securityStamp)
        SQL)->execute([
            'id' => $memberUserId, 'orgId' => $seeded['orgId'], 'username' => $username, 'normalizedUsername' => $username,
            'hash' => 'x', 'displayName' => 'Teammate', 'securityStamp' => Uuid::v4(),
        ]);
