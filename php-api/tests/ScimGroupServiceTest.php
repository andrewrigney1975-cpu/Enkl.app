...
final class ScimGroupServiceTest extends TestCase
{
    ...
    private static function seedScimUser(string $orgId, string $displayName): string
    {
        ...
        self::$db->prepare(<<<SQL
            ...
            VALUES (:id, :orgId, :username, :username, 'x', :displayName, false, false, true, now())
        SQL)->execute(['id' => $userId, 'orgId' => $orgId, 'username' => TestDataHelper::unique('scimuser'), 'displayName' => $displayName]);
        return $userId;
    }
