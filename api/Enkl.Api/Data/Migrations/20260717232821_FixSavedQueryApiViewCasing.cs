...
{
    /// <inheritdoc />
    // Real bug found live (2026-07-18, second report, same day as FixSavedQueryApiViews): a saved
    // query need not bracket-quote every identifier — `t.columnId` (bare, no brackets) resolves
    // fine in AlaSQL, matching the JS object property exactly as typed. Postgres, in contrast, folds
    // an UNQUOTED identifier to lowercase before matching against a real column — so the previous
    // migration's mixed-case, double-quoted columns (`"columnId"`) were invisible to a bare
    // `t.columnId` reference (Postgres was looking for `columnid`). Fixed by making every view name
    // and column fully lowercase, paired with PublicQueryExecutionService's
    // TranslateBracketIdentifiers now ALSO lowercasing bracket contents (not just quoting them) —
    // so a bare `columnId` (auto-folded by Postgres) and a bracketed `[columnId]` (translated to
    // `"columnid"`) both resolve to the identical column.
    public partial class FixSavedQueryApiViewCasing : Migration
    {
        /// <inheritdoc />
