...
{
    /// <inheritdoc />
    // Real bug found live (2026-07-18): the original ten `query_*` views (AddSavedQueryApiExposure)
    // did `SELECT *`, preserving the real Postgres PascalCase column names — but a saved query is
    // authored against the AlaSQL grammar (query-engine.js's TABLE_SCHEMAS), which uses lowercase
    // table names and camelCase field names (`columnId`, not `ColumnId`). Combined with the separate
    // bracket-quoting bug fixed in PublicQueryExecutionService, EVERY saved query beyond a bare
    // `SELECT 1` failed. Fix: drop the old views, recreate with exact-cased names matching AlaSQL's
    // table names (`"tasks"`, `"taskTypes"`, ... — double-quoted at creation so Postgres doesn't
    // fold them to lowercase) and every column aliased to its TABLE_SCHEMAS camelCase name.
    //
    // Fields backed by a junction table client-side (dependencies, documentIds, principleIds,
    // objectiveIds, riskIds, relatedDocumentIds, memberIds) are exposed via a correlated
    // `array_agg` subquery — there's no single column to alias. `members.name`/`members.email` come
    // from a JOIN to "Users" (ProjectMember itself has no display name/email — see MemberDto), which
    // works without granting enkl_public_query direct access to "Users": Postgres views run with the
    // VIEW OWNER's privileges by default (security_invoker is off unless set), so the low-privilege
    // role only ever needs SELECT on the view itself. `tasks.isPrivate` has no server-side column at
    // all (a local-storage-only concept, never synced — see mutations.js) and is exposed as a
    // literal NULL so a saved query referencing it doesn't error, matching AlaSQL's own
    // undefined-property-is-undefined behavior for a field that was never set on the object.
    public partial class FixSavedQueryApiViews : Migration
    {
        /// <inheritdoc />
